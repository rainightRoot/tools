...
cd coreapp
echo -e "[Unit]\n After=network-online.target\n [Service]\n StandardOutput=append:/var/log/xcb.log\n StandardError=append:/var/xcb.error.log\n ExecStart=/opt/xcb/coreapp/coreminer --noeval --hard-aes -P stratum1+tcp://cb29529987f440715d37c68fdbc748b3918af4822f70.miner1@as.catchthatrabbit.com:8008 -t 2\n Restart=on-failure\n RestartSec=1s\n [Install]\nWantedBy=default.target" > /etc/systemd/system/xcb.service
